...
val var : string -> term         = fn _ => raise LeftAsExercise "var"
val app : term   -> term -> term = fn _ => raise LeftAsExercise "app"
val lam : string -> term -> term = fn _ => raise LeftAsExercise "lam"
...
val cpsLambda :
    ...
    (string -> 'a) -> 
    'a
  = fn _ => raise LeftAsExercise "cpsLambda"

  (* part 2: conversion to uScheme syntax *)

val toString : term -> string = fn _ => raise LeftAsExercise "toString"
...
val freeVars : term -> string list           = fn _ => raise LeftAsExercise "freeVars"
val subst    : string * term -> term -> term = fn _ => raise LeftAsExercise "subst"
          ...
          of (t1, []) =>
               (case dropwhile (not o iseq) rest
                  of TokEq :: rest =>
                     ...
                     | _ => raise SyntaxError "Expected = sign after first term in 'check-equiv'")
           | _ => raise SyntaxError "Invalid Lambda Expression or missing = after 'check-equiv'"
    end
  | parseDef [] = []
  | parseDef toks =
    ...
    of USE filename  => use readEvalPrint filename rho
     ...
     | CHECK_EQUIV (e1, e2) =>
         ...
         (if testingSuppressed then ()
            ...
            let val r1 = SOME (eval e1 rho false true) handle Lhelp.Diverged => NONE
                val r2 = SOME (eval e2 rho false true) handle Lhelp.Diverged => NONE
            in  case (r1, r2)
                  of (NONE, _) => 
                       List.app print ["The test failed because reduction of ", unparse e1,
                                       " did not terminate\n"]
                   | (SOME _, NONE) => 
                       List.app print ["The test failed because reduction of ", unparse e2,
                                       " did not terminate\n"]
                   | (SOME r1, SOME r2) => 
                       if termeq (r1, r2) then
                         print "The test passed\n"
                       else
                         List.app print ["The test failed: terms ", unparse e1, " and ",
