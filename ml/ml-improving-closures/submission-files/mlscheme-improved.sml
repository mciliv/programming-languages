...
fun predefinedFunctionError s = eprintln
  ("while reading predefined functions, " ^ s)
...
in
  ...
  fun applyCheckingOverflow f =
    ...
    else if false andalso !evalFuel <= 0 then (* Edited *)
...
(* definition of [[expString]] for \uscheme 1101c *)
fun expString e =
  let fun bracket s = "(" ^ s ^ ")"
...
fun freeIn (VAR x) n = (x = n)
  | freeIn (SET (x, e)) n = (x = n) orelse (freeIn e n)
  | freeIn (IFX (e1, e2, e3)) n = (freeIn e1 n) orelse (freeIn e2 n) orelse                              (freeIn e3 n)  
  | freeIn (WHILEX (e1, e2)) n = (freeIn e1 n) orelse (freeIn e2 n)

  | freeIn (BEGIN elist) n = List.exists (fn (e) => freeIn e n) elist

  | freeIn (APPLY  (e1, elist)) n = (freeIn e1 n) orelse
                                    (freeIn (BEGIN elist) n)

  | freeIn (LAMBDA (args, e)) n = (freeIn e n) andalso 
                                  not (List.exists (fn (x) => x = n) args)
                                  

  | freeIn (LETX (LET, nelist, exp)) n  = 
          (freeIn exp n) andalso 
          not (List.exists (fn (x, e) => (x = n)) nelist) orelse
          List.exists (fn (x, e) => freeIn e n) nelist
                                       
  | freeIn (LETX (LETREC, nelist, exp)) n  =
        not (List.exists (fn (x, e) => (x = n)) nelist) andalso 
        (freeIn exp n) orelse 
        not (List.exists (fn (x, e) => (x = n)) nelist) andalso
        List.exists (fn (x, e) => freeIn e n) nelist
  
  | freeIn (LETX (LETSTAR, [], e)) n  = freeIn e n
  | freeIn (LETX (LETSTAR, xe::xes, e)) n =
                    freeIn (LETX (LET, [xe], LETX (LETSTAR, xes, e))) n 
  | freeIn _ _ = false

val () =
        Unit.checkAssert "VAR 1"
        (fn () => freeIn (VAR "x") "x")

val () =
        Unit.checkAssert "LAMBDA (a) 1"
...
val () =
        Unit.checkAssert "LAMBDA (b) 1"
        (fn () => freeIn (LAMBDA (["y"], (APPLY (VAR ("equal?"),
                            [VAR "x", VAR "y"])))) 
                          "equal?")
...
val () =
        Unit.checkAssert "LAMBDA (b) 1"
        (fn () => not (freeIn (LAMBDA (["y"], (APPLY (VAR ("equal?"),
                            [VAR "x", VAR "y"])))) 
                          "y"))

fun improve ((nlist, e), env) =
  let val newenv = List.filter
    (fn (ni, vi) => freeIn (LAMBDA (nlist, e)) ni) env
  in ((nlist, e), newenv)
  end
...
(* lexical analysis and parsing for \uscheme, providing [[filexdefs]] and [[stringsxdefs]] 1095a *)
...
(* parsers and [[xdef]] streams for \uscheme 1099a *)
fun exptable exp =
...
val _ = op bindings  : (name * exp) list parser
  in usageParsers
   ...
   (* rows added to ML \uscheme's [[exptable]] in exercises ((prototype)) 384 *)
     , ("(cond ([q a] ...))",
     ...
     (* rows added to ML \uscheme's [[exptable]] in exercises 1099b *)
     (* add syntactic sugar here, each row preceded by a comma *)
     ]
  end
(* parsers and [[xdef]] streams for \uscheme 1099d *)
val exp = fullSchemeExpOf (atomicSchemeExpOf name) exptable
(* parsers and [[xdef]] streams for \uscheme 1100a *)
...
(* parsers and [[xdef]] streams for \uscheme 1100b *)
...
(* parsers and [[xdef]] streams for \uscheme 1100c *)
val xdeftable = usageParsers
  [ ("(use filename)", USE <$> name)
  (* rows added to \uscheme\ [[xdeftable]] in exercises 1100d *)
  (* add syntactic sugar here, each row preceded by a comma *) 
  ]
(* type declarations for consistency checking *)
val _ = op xdeftable : xdef parser
(* parsers and [[xdef]] streams for \uscheme 1100e *)
val xdef =  DEF  <$> deftable
        ...
        <?> "definition"
(* type declarations for consistency checking *)
(* parsers and [[xdef]] streams for \uscheme 1100f *)
val xdefstream = 
...
(* evaluation, testing, and the read-eval-print loop for \uscheme 371e *)




(* definitions of [[eval]], [[evaldef]], [[basis]], and [[processDef]] for \uscheme 364c *)
fun eval (e, rho) =
  let val go = applyCheckingOverflow id in go end (* OMIT *)
  let fun ev (LITERAL v) = v
        (* more alternatives for [[ev]] for \uscheme 364d *)
        | ev (VAR x) = !(find (x, rho))
        | ev (SET (x, e)) = 
            let val v = ev e
            in  find (x, rho) := v;
        ...
        (* more alternatives for [[ev]] for \uscheme 365d *)
...
(* definitions of [[eval]], [[evaldef]], [[basis]], and [[processDef]] for \uscheme 370 *)
...
(* definitions of [[eval]], [[evaldef]], [[basis]], and [[processDef]] for \uscheme 373b *)
...
(* definition of [[testIsGood]] for \uscheme 1101a *)
...
(* implementations of \uscheme\ primitives and definition of [[initialBasis]] 374c *)
...
(* utility functions for building primitives in \uscheme 369c *)
