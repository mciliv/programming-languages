...
(* abstract syntax and values for \timpcore 394d *)
(* definitions of [[exp]] and [[value]] for \timpcore 393d *)
datatype value = NUM   of int
               | ARRAY of value array
(* definitions of [[exp]] and [[value]] for \timpcore 393e *)
datatype exp   = LITERAL of value
               ...
               (* array extensions to \timpcore's abstract syntax 410a *)
               | AAT   of exp * exp
               | APUT  of exp * exp * exp
               | AMAKE of exp * exp
               | ASIZE of exp
(* definition of type [[func]], to represent a \timpcore\ function 394c *)
datatype func = USERDEF   of name list * exp
              | PRIMITIVE of value list -> value
(* definition of [[def]] for \timpcore 394a *)
type userfun = { formals : (name * ty) list, body : exp, returns : ty }
datatype def = VAL    of name * exp
             | EXP    of exp
             | DEFINE of name * userfun
(* definition of [[unit_test]] for \timpcore 394b *)
...
(* definition of [[valueString]] for \timpcore 1109a *)
...
(* definition of [[expString]] for \timpcore 1108a *)
fun expString e =
  let fun bracket s = "(" ^ s ^ ")"
...
(* definitions of [[defString]] and [[defName]] for \timpcore 1108b *)
fun defString d =
  let fun bracket s = "(" ^ s ^ ")"
      val bracketSpace = bracket o spaceSep
...
(* abstract syntax and values for \timpcore 410b *)
...
(* utility functions on \timpcore\ values 1105 *)
    ...
    (case ty a
      ...
      (case ty i
        ...
        (case eqType(ty e, t)
         ...
         | _ => raise TypeError ("Error"))
       | _ => raise TypeError ("Error"))
  | ty (AMAKE (len, init)) = case ty len
                                  of INT => ARRAYTY init
                                  | _    => raise TypeError("Error")
  | ty (ASIZE a) = case ty a
                      of ARRAYTY t => INTTY
                      | _    => raise TypeError("Error")
*)
...
(* type checking for \timpcore 404b *)
fun elabdef (d, globals, functions) =
    case d
      of (* cases for elaborating definitions in \timpcore 404c *)
           VAL (x, e) =>
         ...
         (* cases for elaborating definitions in \timpcore 405a *)
         | EXP e =>
             let val tau = typeof (e, globals, functions, emptyEnv)
             in  (globals, functions, typeString tau)
             end
         (* cases for elaborating definitions in \timpcore 405b *)
...
(* lexical analysis and parsing for \timpcore, providing [[filexdefs]] and [[stringsxdefs]] 1109b *)
...
(* parsers and [[xdef]] streams for \timpcore 1110a *)
...
(* parsers and [[xdef]] streams for \timpcore 1111a *)
...
(* parsers and [[xdef]] streams for \timpcore 1111b *)
...
(* parsers and [[xdef]] streams for \timpcore 1111c *)
...
(* parsers and [[xdef]] streams for \timpcore 1111d *)
val xdeftable = usageParsers
  [ ("(use filename)", USE <$> name)
  (* rows added to \timpcore\ [[xdeftable]] in exercises 1111f *)
...
val xdef =  DEF  <$> deftable 
        ...
        <|> DEF <$> EXP <$> exp
        <?> "definition"
(* parsers and [[xdef]] streams for \timpcore 1111e *)
...
(* evaluation, testing, and the read-eval-print loop for \timpcore 1112a *)
(* definitions of [[eval]] and [[evaldef]] for \timpcore 1112b *)
val unitVal = NUM 1983
(* definitions of [[eval]] and [[evaldef]] for \timpcore 1113a *)
...
fun eval (e, globals, functions, formals) =
  ...
  let val toBool = bool
      ...
      fun ev (LITERAL n)          = n
        ...
        | ev (APPLY (f, args)) =
            (case find (f, functions)
               of PRIMITIVE p  => p (map ev args)
                | USERDEF func =>
                    (* apply user-defined function [[func]] to [[args]] 1113b *)
        ...
        (* more alternatives for [[ev]] for \timpcore 410c *)
        | ev (AAT (a, i)) =
...
(* definitions of [[eval]] and [[evaldef]] for \timpcore 1114a *)
fun evaldef (d, globals, functions) =
  case d 
    of VAL (x, e) => (* evaluate [[e]] and bind the result to [[x]] 1114b *)
...
val _ = op evaldef : def * value ref env * func env -> value ref env * func env
                                                                        * string
(* definitions of [[basis]] and [[processDef]] for \timpcore 406a *)
...
(* definition of [[testIsGood]] for \timpcore 1106a *)
        ...
        | checkTypeErrorPasses d =
            (let val t = deftystring d
             in  failtest ["check-type-error failed: expected ", defString d,
                      ...
                      | Located (_, TypeError _) => true)
      (* definition of [[checkFunctionTypePasses]] 1106b *)
...
(* implementations of \timpcore\ primitives and definition of [[initialBasis]] 407a *)
...
(* utility functions and types for making \timpcore\ primitives 1114e *)
val arithtype = FUNTY ([INTTY, INTTY], INTTY)
(* type declarations for consistency checking *)
val _ = op arithtype : funty
(* utility functions and types for making \timpcore\ primitives 1115a *)
...
val initialBasis =
  let fun addPrim ((name, prim, funty), (tfuns, vfuns)) = 
      ...
      val (tfuns, vfuns)  = foldl addPrim (emptyEnv, emptyEnv)
                            (
                           (* primitive functions for \timpcore\ [[::]] 1114f *)
                             ("+", arithOp op +,   arithtype) :: 
                             ("-", arithOp op -,   arithtype) :: 
                             ("*", arithOp op *,   arithtype) :: 
                             ("/", arithOp op div, arithtype) ::

                           (* primitive functions for \timpcore\ [[::]] 1114g *)
                           ...
                           (* primitive functions for \timpcore\ [[::]] 1115b *)
