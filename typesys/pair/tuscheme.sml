...
(* exceptions used in languages with type checking 979b *)
...
fun atLoc loc f a =
  f a handle e as RuntimeError _ => raise Located (loc, e)
           ...
           (* more handlers for [[atLoc]] ((type-checking)) 406c *)
...
(* sets of free type variables in \tuscheme 441 *)
fun freetyvars t =
...
(* sets of free type variables in \tuscheme 442 *)
...
(* shared utility functions on sets of type variables 464 *)
...
(* kind checking for {\tuscheme} 447e *)
fun kindof (tau, Delta) =
  let (* definition of internal function [[kind]] 448a *)
      fun kind (TYVAR a) =
            (find (a, Delta)
             handle NotFound _ => raise TypeError ("unknown type variable " ^ a)
                                                                               )
      (* definition of internal function [[kind]] 448b *)
        | kind (TYCON c) =
            (find (c, Delta)
             handle NotFound _ => raise TypeError ("unknown type constructor " ^
                                                                             c))
      (* definition of internal function [[kind]] 448c *)
      ...
      (* definition of internal function [[kind]] 448d *)
               ...
               of ARROW (formal_kinds, result_kind) =>
                ...
                | TYPE =>
                    raise TypeError ("tried to apply type " ^ typeString tau ^
                                     " as type constructor"))
      (* definition of internal function [[kind]] 449a *)
...
(* kind checking for {\tuscheme} 449b *)
fun asType (ty, Delta) =
...
(* abstract syntax and values for {\tuscheme} 429b *)
(* definitions of [[exp]] and [[value]] for {\tuscheme} 428a *)
...
(* definitions of [[exp]] and [[value]] for {\tuscheme} 428b *)
and    value = NIL
             | BOOLV     of bool   
             | NUM       of int
             | SYM       of name
...
(* definition of [[def]] for {\tuscheme} 428c *)
datatype def  = VAL    of name * exp
              | VALREC of name * tyex * exp
              | EXP    of exp
              | DEFINE of name * tyex * lambda_exp
(* definition of [[unit_test]] for explicitly typed languages 428d *)
...
(* definition of [[expString]] for {\tuscheme} 1127a *)
fun expString e =
  let fun bracket s = "(" ^ s ^ ")"
...
(* definitions of [[defString]] and [[defName]] for {\tuscheme} 1127b *)
fun defString d =
  let fun bracket s = "(" ^ s ^ ")"
      val bracketSpace = bracket o spaceSep
...
(* capture-avoiding substitution for {\tuscheme} 444 *)
fun tysubst (tau, varenv) =
  let
  (* definition of [[renameForallAvoiding]] for {\tuscheme} ((prototype)) 463 *)
...
(* substitute in [[tau]] as specified by [[varenv]], without capturing or substituting for any [[alphas]] 445b *)
...
(* substitute [[varenv]] in [[FORALL (alphas, tau)]] (works only if there is no capture) 445a *)
...
(* capture-avoiding substitution for {\tuscheme} 445c *)
...
(* capture-avoiding substitution for {\tuscheme} 446a *)
...
(* type equivalence for {\tuscheme} 439 *)
(* infinite supply of type variables 440b *)
val infiniteTyvars = 
...
(* Boolean saying if \monobox{FORALL (alphas, tau)} $\equiv$ \monobox{FORALL (alphas', tau')} 440a *)
...
(* type checking for {\tuscheme} ((prototype)) 434 *)
fun typeof (e, kind, gamma)  = 
  let 
      fun literal (NUM n) = inttype
        | literal (BOOLV n) = booltype
        | literal (SYM n) = symtype
        | literal (NIL) = FORALL(["'a"], listtype tyvarA)
        | literal (PAIR (x, xs)) =
          let val tau = literal x
          in case xs
              of NIL => listtype (tau)
               | PAIR (fst, rest) =>
                  if eqType (listtype (tau), literal xs)
                    then listtype (tau)
                    else raise TypeError
                              ("PAIR: Cannot have different types")
                | _ => raise TypeError
                            ("PAIR: Second value must be pair or nil")
          end

        | literal _ = 
            raise TypeError ("Literal: Invalid Literal")

      fun ty (LITERAL v) = literal v
        | ty (IFX (e1, e2, e3)) =
            if eqType (ty e1, booltype) then
              let val (t2, t3) = (ty e2, ty e3)
              in if eqType (t2, t3) then t2 else raise TypeError
                ("IFX: type mismatch of ") (*^ e2 ^ " and " ^ e3)*)
              end
            else
              raise TypeError ("IFX: ") (*^ e1 ^ "is not type bool")*)
        | ty (SET (x, e)) =
            let val tau_x = find (x, gamma)
                val tau_e = ty e
            in if eqType (tau_x, tau_e) then tau_e else raise TypeError ("SET invalid")
            end
        | ty (VAR x) =
            let val tau_x = find (x, gamma)
            in tau_x
        ...
        | ty (APPLY (f, actuals)) =
            ...
            in case typeof (f, kind, gamma)
                 of FUNTY (args, result) =>
                   if eqTypes (atys, args) then
                     result
                   else
                     raise TypeError ("APPLY: Params and actuals not equal")
                   | _ => raise TypeError ("APPLY: Not proper function type")
            end
        | ty (LETX (LET, bs, e)) =
            let val (names, values) = ListPair.unzip bs
            in  typeof (e, kind, bindList (names, map ty values, gamma))
            end
        | ty (LAMBDA (args, e)) =
            let val (names, taus) = ListPair.unzip args
            in FUNTY (taus, typeof (e, kind, bindList (names,
              map (fn (t) => asType (t, kind)) taus, gamma)))
            end
        | ty (WHILEX (e1, e2)) =
            let val tau = typeof(e2, kind, gamma)
            in if eqType (ty e1, booltype) 
                 then unittype
                 else raise TypeError ("While: E1 is not bool")
            end
        | ty (BEGIN (es)) = 
          let fun b (e::es, lastval) =  b (es, typeof (e, kind, gamma))
                  | b ( [], lastval) = lastval
                in b (es, unittype)
            end
        | ty (LETX (LETSTAR, es, body)) = 
            let fun add_tuples ((n, e), gamma) = bind (n, typeof (e, kind, gamma), gamma)
            in typeof (body, kind, foldl add_tuples gamma es)
            end
        | ty (LETRECX (es, body)) =
            let val (names, expressions) = ListPair.unzip es
                val (vars, types) = ListPair.unzip names
                fun check_lambda (list_es) =
                  (List.all (fn (e) => case e of
                                         LAMBDA (_) => true
                                          | _ => false) list_es)
                val kind_list = map (fn (t) => asType (t, kind)) types
                val gamma = bindList (vars, types, gamma)
                val typeof_body = typeof (body, kind, gamma)  
            in if check_lambda (expressions)
                then typeof_body
                else raise TypeError ("LETRECX: Not all are lambdas")
            end
        | ty (TYLAMBDA (names, exp)) = 
          let val free_variables = freetyvarsGamma gamma
              fun is_in (n, fvs) = List.exists (fn (fv) => n = fv) fvs
              fun no_elems_fvs (ns, fvs) =
                not (List.exists (fn (n) => is_in(n, fvs)) ns)
              fun bind_kind (k, ns) =
                List.foldl (fn (x, ys) => (x, TYPE)::ys) k ns

              in if (no_elems_fvs (names, free_variables))
                   then
                     FORALL(names, typeof(exp, bind_kind(kind, names), gamma))
                   else
                     raise TypeError ("TYLAMBDA: a type variable is free")
              end
        | ty (TYAPPLY (e, types)) =
          (*let val typeof_exp = typeof (e, kind, gamma)*)
           case ty e
            of FORALL (_) => instantiate (ty e, types, kind)

            | _ => raise TypeError ("TYAPPLY: Typeof expression is not forall")
...
fun elabdef (e, kind, gamma) =
  case e
    of
      VAL (x, e) =>
        let val tau = typeof (e, kind, gamma)
    ...
    (* Do the premises in order because they can depend on each other *) 
    | VALREC (x, tau, e) =>
        let val tau = asType (tau, kind)
            val gamma = bind (x, tau, gamma)
            val e_type = typeof (e, kind, gamma)
          in case e
              of
              LAMBDA (_) =>
                (gamma, typeString tau)
              | _ => raise TypeError ("VALREC: Expression is not a lambda")
          end
    | DEFINE (f, tau, (lambda, e)) =>
        let val (vars, types) = ListPair.unzip lambda

        in elabdef
          (VALREC(f, FUNTY(types, tau), LAMBDA(lambda, e)), kind, gamma)
        end
...
(* lexical analysis and parsing for \tuscheme, providing [[filexdefs]] and [[stringsxdefs]] 1117b *)
...
(* parser builders for typed languages 1110b *)
...
(* parsers and [[xdef]] streams for {\tuscheme} 1118a *)
fun keyword words =
...
(* parsers and [[xdef]] streams for {\tuscheme} 1118d *)
...
(* parsers and [[xdef]] streams for {\tuscheme} 1119 *)
...
(* parsers and [[xdef]] streams for {\tuscheme} 1120b *)
val unit_test =
...
(* parsers and [[xdef]] streams for {\tuscheme} 1120c *)
val xdef = 
     DEF <$> def
...
(* evaluation, testing, and the read-eval-print loop for {\tuscheme} 1120e *)
...
(* definitions of [[eval]] and [[evaldef]] for {\tuscheme} 1121a *)
fun eval (e, rho) =
  let val go = applyCheckingOverflow id in go end (* OMIT *)
  let fun ev (LITERAL n) = n
        (* alternatives for [[ev]] for [[TYAPPLY]] and [[TYLAMBDA]] 450 *)
        | ev (TYAPPLY  (e, _)) = ev e
        | ev (TYLAMBDA (_, e)) = ev e
        (* more alternatives for [[ev]] for {\tuscheme} 1121b *)
        | ev (VAR v) = !(find (v, rho))
        | ev (SET (n, e)) = 
            let val v = ev e
            in  find (n, rho) := v;
        ...
        (* more alternatives for [[ev]] for {\tuscheme} 1121c *)
        ...
        (* more alternatives for [[ev]] for {\tuscheme} 1121d *)
        | ev (LAMBDA (args, body)) = CLOSURE ((map (fn (x, ty) => x) args, body)
                                                                          , rho)
        (* more alternatives for [[ev]] for {\tuscheme} 1121e *)
        | ev (APPLY (f, args))  = 
               (case ev f
                  of PRIMITIVE prim => prim (map ev args)
                   | CLOSURE clo =>
                       (* apply closure [[clo]] to [[args]] ((mlscheme)) 365e *)
        ...
        (* more alternatives for [[ev]] for {\tuscheme} 1122a *)
        | ev (LETX (LET, bs, body)) =
            let val (names, values) = ListPair.unzip bs
        ...
        (* more alternatives for [[ev]] for {\tuscheme} 1122b *)
        | ev (LETRECX (bs, body)) = 
...
(* definitions of [[eval]] and [[evaldef]] for {\tuscheme} 1122c *)
...
(* definitions of [[basis]] and [[processDef]] for {\tuscheme} 451a *)
...
(* other handlers that catch non-fatal exceptions and pass messages to [[caught]] ((type-checking)) 406b *)
...
fun testIsGood (test, (Delta, Gamma, rho)) =
 ...
 (* shared [[check{Expect,Assert,Error,Type}Checks]], which call [[ty]] 1107c *)
 ...
 (* shared [[check{Expect,Assert,Error,Type}Checks]], which call [[ty]] 1126d *)
...
(* shared [[checkTypePasses]] and [[checkTypeErrorPasses]], which call [[ty]] 1106c *)
...
(* shared [[checkTypePasses]] and [[checkTypeErrorPasses]], which call [[ty]] 1107a *)
...
(* implementations of \tuscheme\ primitives and definition of [[initialBasis]] 451b *)
(* shared utility functions for building primitives in languages with type checking 1114c *)
...
(* shared utility functions for building primitives in languages with type checking 1114d *)
fun arithOp f =
...
(* utility functions and types for making \tuscheme\ primitives 1123b *)
val arithtype = FUNTY ([inttype, inttype], inttype)
(* type declarations for consistency checking *)
val _ = op arithtype : tyex
(* utility functions and types for making \tuscheme\ primitives 1123d *)
...
val initialBasis =
  let fun addKind ((name, kind), kinds) = bind (name, kind, kinds)
      val kinds   = foldl addKind emptyEnv
                    (
                    (* primitive type constructors for \tuscheme\ [[::]] 421c *)
                     ("int",  TYPE) ::
                     ("bool", TYPE) ::
                     ("sym",  TYPE) ::
                     ("unit", TYPE) ::
      ...
      val (types, values) = foldl addPrim (emptyEnv, emptyEnv)
                            (
                           (* primitive functions for \tuscheme\ [[::]] 1123c *)
                             ("+", arithOp op +,   arithtype) :: 
                             ("-", arithOp op -,   arithtype) :: 
                             ("*", arithOp op *,   arithtype) :: 
                             ("/", arithOp op div, arithtype) ::

                           (* primitive functions for \tuscheme\ [[::]] 1123e *)
                           ...
                           (* primitive functions for \tuscheme\ [[::]] 1124a *)
                             ...
                             ("empty?", unaryOp (BOOLV o (fn (NIL   ) => true | _
                                              => false))
             , FORALL (["'a"], FUNTY ([queuetype tyvarA],
                                          booltype))) ::

                  ("get-first",  unaryOp  (fn (PAIR (car, _)) => car 
                                       | v => raise RuntimeError
                                                      (
                  ...
                  ("get-rest",  unaryOp  (fn (PAIR (_, cdr)) => cdr 
                                       | v => raise RuntimeError
                                                      (
              ...
              ("put", binaryOp let fun put (x, NIL)          = PAIR (x, NIL)
                      ...
                      "non-queue passed to put" in put end, 
                    FORALL (["'a"], FUNTY (
                      [queuetype tyvarA], queuetype tyvarA))) :: 

                           (* primitive functions for \tuscheme\ [[::]] 1124b *)
      ...
      val (types, values) =
                      ...
                      (
             (* primitives that aren't functions, for \tuscheme\ [[::]] 1124c *)
