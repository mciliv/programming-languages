...
fun typeof _  = 
  (* type checking for \timpcore 400a *)
fun typeof (e, globals, functions, formals) =
  let
 (* function [[ty]], to check the type of an expression, given $\itenvs$ 400b *)
      fun ty (LITERAL v) = INTTY

 (* function [[ty]], to check the type of an expression, given $\itenvs$ 401a *)
        | ty (VAR x) = (find (x, formals) handle NotFound _ => find (x, globals)
                                                                               )

 (* function [[ty]], to check the type of an expression, given $\itenvs$ 401b *)
        | ty (SET (x, e)) =
 ...
 (* function [[ty]], to check the type of an expression, given $\itenvs$ 402a *)
        | ty (IFX (e1, e2, e3)) =
            let val tau1 = ty e1
                val tau2 = ty e2
                val tau3 = ty e3
 ...
 (* function [[ty]], to check the type of an expression, given $\itenvs$ 402c *)
        | ty (BEGIN es) =
 ...
 (* function [[ty]], to check the type of an expression, given $\itenvs$ 403c *)
        | ty (APPLY (f, actuals)) =
             let val actualtypes                     = map ty actuals
                 val FUNTY (formaltypes, resulttype) = find (f, functions)
                 (* definition of [[parameterError]] 404a *)
...
(* function [[ty]], to check the type of an expression, given $\itenvs$ ((prototype)) 411 *)
      | ty (AAT (a, i)) =
        (case (ty a, ty i)
          of (ARRAYTY t, INTTY) => t
